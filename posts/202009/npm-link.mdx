...
__Note__: If you have your own react project and library you are currently working on, you can try the workflow in your project.
...
If you face problems with permissions, you can run the npm link command with sudo or [change the global _node_modules_ to another path](http://npm.github.io/installation-setup-docs/installing/a-note-on-permissions.html) that does not require root privileges.
...
Now, whenever you change anything in your project it gets reflected into the lib present in the global _node_modules_ folder which in turn gets reflected in the project on which you are currently working on.

__Note__: If you distribute your package after building it then make sure that config which are used to build the package are also in your project's build config.